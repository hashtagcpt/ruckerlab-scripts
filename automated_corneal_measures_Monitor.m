...
% debug params
plot_fig = 0; % do not do debug plots

% factor - what portion of the central image to use
factor = 64;

% set working directory for image files
wd = '/media/tiny/ruckerlab/KERA/Falk_Matt_Kera/none'; %% edit here %%

light_d = {'white' , 'yellow'}; %% edit here -- condition1 %%
freq_d = {''}; %% edit here -- condition2 %%
pre_post_str = {'PRE', 'POST'};
eye = {'OD','OS'}

% create a raw datafile and header
big_filename = 'monitor_kera_all.csv';  %% edit here %%
...
for brCtr = 1:length(light_d)
    for freqCtr = 1:length(freq_d)
        for pre_post = 1:length(pre_post_str)
            % change to the wd
            cd(sprintf('%s/%s_%s', wd, freq_d{freqCtr}, light_d{brCtr}));
            fprintf('dir: %s\n', pwd);
            if pre_post == 1    
                % get pre files
                im_dir= dir('*PRE*');
...
for brCtr = 1:length(light_d)
    for freqCtr = 1:length(freq_d)
        ...
        for avgCTR = 2:2:size(dPRE,1)
            ...
            Light(ctr) = brCtr;
            Freq(ctr) = str2num(freq_d{freqCtr});
            ctr = ctr + 1;
        end
